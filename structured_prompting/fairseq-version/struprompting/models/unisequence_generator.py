...
class UniSequenceGenerator(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        if isinstance(models, UniEnsembleModel):
            self.model = models
        else:
            self.model = UniEnsembleModel(models)
        ...
        self.external_qkv = external_qkv
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if step >= len(prefix_tokens[0]):
                eos_mask = cand_indices.eq(self.eos) & cand_scores.ne(-math.inf)
            else:
                eos_mask = cand_indices.eq(-1) & cand_scores.ne(-math.inf)
...
class UniEnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        if not begin:
            ...
            decoder_out_tuple = decoder_out[0].div_(temperature)
